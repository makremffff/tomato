...
async function _doBootstrap() {
  ...
  try {
    ...
    await sql(`CREATE TABLE IF NOT EXISTS monetag_logs (
      id BIGSERIAL PRIMARY KEY,
      user_id BIGINT NOT NULL,
      log_date DATE NOT NULL DEFAULT CURRENT_DATE,
      count INT DEFAULT 0, points_earned BIGINT DEFAULT 0,
    ...
    const migrations = [
      ...
      `ALTER TABLE users     ADD COLUMN IF NOT EXISTS monetag_last_reward TIMESTAMPTZ`,
    ...
    // ── Social Tasks Tables ──────────────────────────────────────
    await sql(`CREATE TABLE IF NOT EXISTS social_tasks (
      id         SERIAL PRIMARY KEY,
      title      TEXT NOT NULL,
      ...
      promo_optional BOOLEAN DEFAULT TRUE,
    ...
    await sql(`CREATE TABLE IF NOT EXISTS social_proofs (
      id          BIGSERIAL PRIMARY KEY,
      user_id     BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      task_id     INT    NOT NULL REFERENCES social_tasks(id) ON DELETE CASCADE,
    ...
    await sql(`CREATE INDEX IF NOT EXISTS idx_sp_status  ON social_proofs(status)`);
    await sql(`CREATE INDEX IF NOT EXISTS idx_sp_user_id ON social_proofs(user_id)`);
    // seed default tasks if empty
    await sql(`
      INSERT INTO social_tasks(title, description, reward, icon, note, promo_text, task_url, proof_required, sort_order)
      SELECT * FROM (VALUES
        ('انشر البوت على فيسبوك',      'شارك الرابط في غروبك أو صفحتك',       500, 'facebook',
         'انسخ المنشور أدناه وشاركه في صفحتك أو مجموعتك ثم أرسل لقطة شاشة لإثبات التنفيذ.',
         '💰 اربح مال حقيقي من هاتفك!\n\nشاهد إعلانات، أدِّ مهام بسيطة، وادعُ أصدقاءك — واسحب أرباحك بـ USDT في أي وقت 🚀\n\n👇 سجّل مجاناً الآن\nhttps://t.me/RebhApp_bot',
         'https://www.facebook.com/', TRUE, 0),
        ('تفاعل مع آخر منشور في تيليغرام', 'أضف رد فعل على منشور القناة',       75, 'telegram',
         'افتح قناة @RebhApp على تيليغرام وأضف رد فعل على آخر منشور ثم أرسل لقطة الإثبات.',
         '', 'https://t.me/RebhApp', TRUE, 1)
      ) AS v(title, description, reward, icon, note, promo_text, task_url, proof_required, sort_order)
      WHERE NOT EXISTS (SELECT 1 FROM social_tasks LIMIT 1)
    `);

    // ── Competition Tables ───────────────────────────────────────
    await sql(`CREATE TABLE IF NOT EXISTS competitions (
      id           SERIAL PRIMARY KEY,
      title        TEXT NOT NULL DEFAULT 'بطولة التذاكر',
      prize_1      TEXT NOT NULL DEFAULT '5$',
      prize_2      TEXT NOT NULL DEFAULT '3$',
      prize_3      TEXT NOT NULL DEFAULT '2$',
    ...
    await sql(`CREATE INDEX IF NOT EXISTS idx_ct_comp_tickets ON competition_tickets(competition_id, tickets DESC)`);
    // seed first competition if none exists
    await sql(`
      INSERT INTO competitions(title, prize_1, prize_2, prize_3, ends_at)
      SELECT 'بطولة التذاكر', '5$', '3$', '2$', NOW() + INTERVAL '14 days'
      WHERE NOT EXISTS (SELECT 1 FROM competitions LIMIT 1)
    `);
