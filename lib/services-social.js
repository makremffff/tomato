...
async function handleSocialSubmitProof(userId, body) {
  ...
  // منع إعادة الإرسال إلا إذا كان الوضع السابق "مرفوض" — في هذه الحالة نسمح بإعادة المحاولة
  if (existing && existing.status !== 'rejected') {
    return { ok: false, error: 'already_submitted' };
  }

  // المهام بدون proof_required تُعتمد فوراً (auto-approve)
  const proofImage    = task.proof_required ? (body?.proof_image || '') : '';
  const initialStatus = task.proof_required ? 'pending' : 'approved';

  if (existing?.status === 'rejected') {
    // إعادة إرسال بعد رفض — UPDATE بدل INSERT
    // لا نعدّل created_at (عمود ثابت)، نصفّر فقط حقول المراجعة
    await sql(
      `UPDATE social_proofs
         SET proof_image = $1, status = $2,
             reviewed_by = NULL, reviewed_at = NULL
       WHERE id = $3`,
      [proofImage, initialStatus, existing.id]
    );
  } else {
    // إرسال أول مرة — نحفظ base64 في حقل proof_image
    // في الإنتاج يُستبدل بـ upload إلى storage مثل S3
    await sql(
      `INSERT INTO social_proofs(user_id, task_id, proof_image, status)
       VALUES($1, $2, $3, $4)`,
      [userId, taskId, proofImage, initialStatus]
    );
  }

  // إذا تمت الموافقة فوراً (مهمة بدون proof) — أضف النقاط مباشرة
  // العمود الصحيح هو points وليس balance (راجع schema في db.js سطر 38)
...
async function handleSocialAdminReview(body) {
  ...
  try {
    ...
    if (action === 'approve') {
      // العمود الصحيح هو points (راجع schema في db.js)
      await sql(
        `UPDATE users SET points = points + $1 WHERE id = $2`,
