...
const CONFIG = Object.freeze({
  withdraw: {
    first_min:    0.01,   // أول سحبة — USDT
    normal_min:   0.02,   // السحب الطبيعي — USDT
    normal_level: 5,      // المستوى المطلوب للسحب الطبيعي
  },
  rewards: {
    referral:          2000,   // تذاكر مسابقة لكل إحالة
    telegram_task:     200,
    adsgram_task:      50,
    daily_ads_10:      200,
    daily_ads_25:      300,
    daily_referrals_3: 1500,
    points_per_ad:     80,    // غير مستخدم
    tickets_per_ad:    500,    // تذاكر لكل إعلان Adsgram
    usdt_per_ad:       0.0005,  // USDT لكل إعلان Adsgram
  },
  telegram: {
    channel_url: 'https://t.me/botbababab',
  },
  ads: {
    daily_limit:       20,     // الحد اليومي للإعلانات العادية
    cooldown_ms:       30 * 1000,
    min_duration_ms:   14 * 1000,
  },
  monetag: {
    daily_limit:   500,    // الحد اليومي لـ Monetag
    reward:        0,     // نقاط لكل مشاهدة
  ...
  referral: {
    min_active_ads:    1,     // الحد الأدنى للإعلانات لاعتبار المستخدم نشيطاً
    min_active_pts:    200,   // أو النقاط
  },
});
...
const CFG = {
  ...
  WITHDRAW_MIN_USDT:     CONFIG.withdraw.normal_min,
  ...
  // ── Competition Tickets ──────────────────────────────────────
  COMPETITION_MAX_TICKETS_PER_DAY: 20000,  // 20 إعلان × 500 + إحالات متعددة
  COMPETITION_MAX_GRANT_PER_CALL:  2000,   // أقصى تذاكر لكل عملية منح واحدة
  ...
  RISK_BAN:              100,
  RISK_SHADOW:           80,   // رُفع من 60 → 80 لتقليل الـ false positives

  // ── Expired Nonce Abuse ───────────────────────────────────────
  // كم مرة expired nonce في ساعة قبل اعتبارها إساءة
  NONCE_EXPIRED_ABUSE_THRESHOLD: 30,
