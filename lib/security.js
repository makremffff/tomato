...
// ═══════════════════════════════════════════════════════════════════
// [FIX-5] لا نستخدم fallback hardcoded — إذا NONCE_SECRET غير موجود في env يوقف السيرفر
const NONCE_HMAC_SECRET = process.env.NONCE_SECRET || (() => {
  if (process.env.NODE_ENV === 'production') {
    throw new Error('[FATAL] NONCE_SECRET env variable is not set in production!');
  }
  // dev فقط — warning واضح
  console.warn('[SECURITY WARNING] NONCE_SECRET not set — using insecure dev fallback');
  return 'dev-only-insecure-fallback-change-in-production';
})();
