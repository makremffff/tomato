...
// ═══════════════════════════════════════════════════════════════════
// ✅ [SEC-FIX] السر مشتق من env vars الموجودة — لا يوجد fallback مكشوف في الكود
// BOT_TOKEN + DATABASE_URL كلاهما سري فلو سرّب المصدر لا يكشف HMAC
const _envSeed = (process.env.BOT_TOKEN || '') + '|' + (process.env.DATABASE_URL || '') + '|nonce-hmac-v1';
const NONCE_HMAC_SECRET = process.env.NONCE_SECRET
  || require('crypto').createHash('sha256').update(_envSeed).digest('hex');

// سر خاص بالـ session IDs — مشتق من BOT_TOKEN
const _sessionSeed = (process.env.BOT_TOKEN || '') + '|session-sign-v1';
const SESSION_SIGN_SECRET = require('crypto').createHash('sha256').update(_sessionSeed).digest('hex');

// ═══════════════════════════════════════════════════════════════════
// SIGNED SESSION ID
// Format: rawId (64 hex) + '.' + hmac_sig (32 hex)
// لا يمكن تزوير session ID حتى لو عرف المهاجم الـ raw part
// ═══════════════════════════════════════════════════════════════════
function genSignedSessionId() {
  const rawId = genRawNonce(32); // 64 hex chars
  const sig   = createHmac('sha256', SESSION_SIGN_SECRET).update(rawId).digest('hex').slice(0, 32);
  return `${rawId}.${sig}`;
}
...
function verifySessionId(signedId) {
  ...
  if (dot < 64) return false; // rawId يجب أن يكون 64 chars
  const rawId = signedId.slice(0, dot);
  const clientSig = signedId.slice(dot + 1);
  const expectedSig = createHmac('sha256', SESSION_SIGN_SECRET).update(rawId).digest('hex').slice(0, 32);
  return constantTimeCompare(clientSig, expectedSig);
}
...
async function validateSession(sid, ipHash, fpHash) {
  if (!sid) return null;
  // ✅ [SEC] تحقق من توقيع الـ session ID قبل أي query للـ DB
  // session ID مزوّر أو قديم (قبل تطبيق الـ signing) يُرفض فوراً
  if (!verifySessionId(sid)) return null;
  const r = await sql(
