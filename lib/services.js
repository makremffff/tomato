...
async function handleCreateSession(body, ipHash, fpHash) {
  ...
  try {
    ...
    if (m) {
      ...
      if (refTgId !== tid) {  // prevent self-referral by same tg_id
        const rr = await sql(`SELECT id FROM users WHERE tg_id=$1`, [refTgId]);
        if (rr.length) {
          const referrerId = rr[0].id;

          // ── Self-referral fraud: same device or same IP ───────────
          const referrerData = (await sql(
            `SELECT fp_hash, last_ip_hash FROM users WHERE id=$1`,
            [referrerId]
          ))[0];

          const sameFp = referrerData?.fp_hash && fpHash && referrerData.fp_hash === fpHash;
          const sameIp = referrerData?.last_ip_hash && ipHash && referrerData.last_ip_hash === ipHash;

          if (sameFp || sameIp) {
            // حساب ثاني — حظر فوري للحساب الجديد والإحالة ملغاة
            await sql(
              `UPDATE users SET is_banned=TRUE, ban_reason='self_referral_fraud', updated_at=NOW() WHERE id=$1`,
              [user.id]
            );
            await writeAudit(user.id, null, 'create_session', 'self_referral_fraud_banned', ipHash, fpHash, {
              tg_id: tid, referrer_id: referrerId, reason: sameFp ? 'same_fp' : 'same_ip',
            });
  ...
  const userRow = (await sql(
    `SELECT is_banned, risk_score, created_at FROM users WHERE id=$1`,
    [user.id]
  ))[0];
...
async function handleLoad(userId) {
  ...
  // ── Active referrals count (for withdraw eligibility) ────────
  const activeRefsRow = await sql(
    `SELECT COUNT(*) as cnt FROM referrals r
     JOIN users u2 ON u2.id = r.referred_id
     WHERE r.referrer_id = $1
       AND (u2.ads_watched >= $2 OR u2.points >= $3)`,
    [userId, CONFIG.referral.min_active_ads, CONFIG.referral.min_active_pts]
  );
  const activeReferralsCount = parseInt(activeRefsRow[0]?.cnt) || 0;

  // ── Withdraw info ─────────────────────────────────────────────
  const firstWithdrawDone = !!u.first_withdraw_done;
  const withdrawInfo = {
    withdraw_min:   CONFIG.withdraw.normal_min,
    withdraw_level: CONFIG.withdraw.normal_level,
    min_active_referrals: CONFIG.withdraw.min_active_referrals,
    first_withdraw_offer: false,
  };
...
async function handleVerifyTgTask(userId, rawNonce, sessionId, fpHash, ipHash) {
  ...
  const uCheck = (await sql(`SELECT tg_id, tg_verified FROM users WHERE id=$1`, [userId]))[0];
  if (!uCheck) return { ok: false, error: 'user_not_found' };
...
// ═══════════════════════════════════════════════════════════════════
// SUBMIT WITHDRAW — 1200pts min | 3 active referrals | once per day
// ═══════════════════════════════════════════════════════════════════
async function handleSubmitWithdraw(userId, body, rawNonce, sessionId, fpHash, ipHash) {
  ...
  // ── 1. Minimum balance check (1200 pts) ───────────────────────
  const minPts = CONFIG.withdraw.normal_min; // 1200
  if (pts < minPts) {
    return { ok: false, error: 'insufficient_balance', required: minPts, have: pts };
  }

  // ── 2. Active referrals check (3 active referrals required) ───
  const minRefs = CONFIG.withdraw.min_active_referrals; // 3
  const activeRefsRes = await sql(
    `SELECT COUNT(*) as cnt FROM referrals r
     JOIN users u ON u.id = r.referred_id
     WHERE r.referrer_id = $1
       AND (u.ads_watched >= $2 OR u.points >= $3)`,
    [userId, CONFIG.referral.min_active_ads, CONFIG.referral.min_active_pts]
  );
  const activeRefs = parseInt(activeRefsRes[0]?.cnt) || 0;
  if (activeRefs < minRefs) {
    return { ok: false, error: 'not_enough_active_referrals', required: minRefs, have: activeRefs };
  }

  // ── 3. Once per day check ─────────────────────────────────────
  const todayRes = await sql(
    `SELECT COUNT(*) as cnt FROM withdrawals
     WHERE user_id = $1 AND created_at >= NOW()::date`,
    [userId]
  );
  if (parseInt(todayRes[0]?.cnt) > 0) {
    return { ok: false, error: 'daily_limit_reached' };
  }

  const ton = parseFloat((pts / CFG.PTS_PER_TON).toFixed(6));
  await sql(`UPDATE users SET points=points-$1, first_withdraw_done=TRUE, updated_at=NOW() WHERE id=$2`, [pts, userId]);
  const wr = await sql(
  ...
  await writeAudit(userId, sessionId, 'submit_withdraw', 'ok', ipHash, fpHash, { pts, ton, first_withdraw: !firstDone });
...
async function handleTrackAdEvent(userId, sessionId, body, ipHash, fpHash) {
  ...
  if (event === 'suspicious_activity') {
    ...
    // log only — no risk score for client-side reports
    console.info(`[TRACK] suspicious_activity userId=${userId} reason=${reason}`);
  }
