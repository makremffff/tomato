...
async function handleGetCompetition(body, userId) {
  // get active competition
  const comps = await sql(`SELECT * FROM competitions WHERE is_active=TRUE ORDER BY id DESC LIMIT 1`);
  if (!comps.length) return { ok: true, competition: null, leaderboard: [], my_rank: null, my_tickets: 0 };
  const comp = comps[0];

  // top 10 leaderboard
  const lb = await sql(`
    SELECT ct.tickets, u.telegram_id, u.username, u.first_name, u.last_name, u.photo_url,
           ROW_NUMBER() OVER (ORDER BY ct.tickets DESC) AS rank
    FROM competition_tickets ct
    JOIN users u ON u.id = ct.user_id
    WHERE ct.competition_id = $1 AND ct.tickets > 0
    ORDER BY ct.tickets DESC
    LIMIT 10
  `, [comp.id]);

  // my rank & tickets
  let myRank = null, myTickets = 0;
  if (userId) {
    const myRow = await sql(`SELECT tickets FROM competition_tickets WHERE user_id=$1 AND competition_id=$2`, [userId, comp.id]);
    myTickets = parseInt(myRow[0]?.tickets) || 0;
    if (myTickets > 0) {
      const rankRow = await sql(`
        SELECT COUNT(*)+1 AS rank FROM competition_tickets
        WHERE competition_id=$1 AND tickets > $2
      `, [comp.id, myTickets]);
  ...
  return {
    ...
    competition: {
      id: comp.id,
      title: comp.title,
      prize_1: comp.prize_1,
      prize_2: comp.prize_2,
      prize_3: comp.prize_3,
      ends_at: comp.ends_at,
    },
    leaderboard: lb.map(r => ({
      rank:       parseInt(r.rank),
      tickets:    parseInt(r.tickets),
      name:       r.first_name ? (r.first_name + (r.last_name ? ' ' + r.last_name : '')) : (r.username || 'مجهول'),
      username:   r.username || '',
...
// Award tickets whenever user earns points (call internally after ad rewards)
async function awardCompetitionTickets(userId, points) {
  try {
    const comps = await sql(`SELECT id FROM competitions WHERE is_active=TRUE ORDER BY id DESC LIMIT 1`);
    if (!comps.length) return;
    const compId = comps[0].id;
    // 1 ticket per 10 points
    const tickets = Math.floor(points / 10);
    if (tickets <= 0) return;
    await sql(`
      INSERT INTO competition_tickets(user_id, competition_id, tickets)
      VALUES($1, $2, $3)
      ON CONFLICT(user_id, competition_id)
      DO UPDATE SET tickets = competition_tickets.tickets + $3, updated_at = NOW()
    `, [userId, compId, tickets]);
  } catch (_) {}
}
