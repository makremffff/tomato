...
async function handleStartAd(userId, sessionId, ipHash, fpHash) {
  ...
  // [FIX-5] خزّن وقت بداية الإعلان على السيرفر — لا نثق بـ elapsed_ms من الكليانت
...
async function handleRewardAd(userId, sessionId, ipHash, fpHash, body, rawNonce) {
  // [FIX-5] نحسب elapsed على السيرفر من ad_started_at — لا نثق بـ ad_elapsed_ms من الكليانت
  const _sessionRow = (await sql(`SELECT ad_started_at FROM sessions WHERE id=$1`, [sessionId]))[0];
  const adElapsedMs = _sessionRow?.ad_started_at
    ? Date.now() - new Date(_sessionRow.ad_started_at).getTime()
    : 0;
  ...
  // [FIX-2] IP binding — نتحقق أن الـ IP نفسه اللي أصدر الـ nonce
  const _ipRow = (await sql(`SELECT ip_hash FROM sessions WHERE id=$1`, [sessionId]))[0];
  if (_ipRow?.ip_hash && _ipRow.ip_hash !== ipHash) {
    await addRisk(userId, 'session_ip_hijack_reward_ad', ipHash, fpHash, 50);
    logRewardBlock('ip_mismatch', userId, { stored: _ipRow.ip_hash?.slice(0,8), current: ipHash?.slice(0,8) });
...
async function handleRewardTaddyAd(userId, sessionId, ipHash, fpHash, body, rawNonce) {
  ...
  // [FIX-2] IP binding
  const _tIpRow = (await sql(`SELECT ip_hash FROM sessions WHERE id=$1`, [sessionId]))[0];
  if (_tIpRow?.ip_hash && _tIpRow.ip_hash !== ipHash) {
    await addRisk(userId, 'session_ip_hijack_taddy', ipHash, fpHash, 50);
    logRewardBlock('ip_mismatch_taddy', userId, { stored: _tIpRow.ip_hash?.slice(0,8), current: ipHash?.slice(0,8) });
    return { ok: false, error: 'session_invalid' };
  }
  // [2] Shadow ban check
  const uR = await sql(`SELECT is_shadow_banned FROM users WHERE id=$1`, [userId]);
...
async function handleClaimAdsgramTask(userId, sessionId, ipHash, fpHash, rawNonce) {
  ...
  // [FIX-2] IP binding
  const _aIpRow = (await sql(`SELECT ip_hash FROM sessions WHERE id=$1`, [sessionId]))[0];
  if (_aIpRow?.ip_hash && _aIpRow.ip_hash !== ipHash) {
    await addRisk(userId, 'session_ip_hijack_adsgram', ipHash, fpHash, 50);
    return { ok: false, error: 'session_invalid' };
  }
