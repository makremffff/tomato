...
// استيراد منح تذاكر المسابقة
const { grantTickets: grantCompTickets } = require('../api/competition');
...
function handleGetConfig() {
  return {
    ...
    rewards: {
      ...
      tickets_per_ad:    CONFIG.rewards.tickets_per_ad,
      usdt_per_ad:       CONFIG.rewards.usdt_per_ad,
    },
    telegram: {
...
async function handleCreateSession(body, ipHash, fpHash) {
  ...
  try {
    ...
    if (sessionCount >= 5) {
      // 5+ sessions في 5 دقائق — بوت أو أتمتة واضحة
  ...
  // جلب تيكيتات المنافسة للموسم الحالي
  let myTickets = 0;
  try {
    const activeSeason = (await sql(
      `SELECT id FROM competition_seasons WHERE end_date > NOW() ORDER BY end_date ASC LIMIT 1`
    ))[0];
    if (activeSeason) {
      ...
      myTickets = parseInt(ticketRow?.tickets) || 0;
    }
  } catch (tkErr) {
    console.warn('[CREATE_SESSION] tickets fetch error (non-fatal):', tkErr.message);
  }
...
async function activatePendingReferral(referredUserId) {
  ...
  const ticketReward = CONFIG.rewards.referral; // 500 تذكرة

  // منح تذاكر المسابقة للمحيل (بدل نقاط)
  try {
    await grantCompTickets(pendingRef.referrer_id, ticketReward);
  } catch (_) {}

  // تحديث عداد الإحالات فقط (بدون إضافة نقاط)
  await sql(
    `UPDATE users
  ...
  await writeAudit(pendingRef.referrer_id, null, 'referral_activated', 'ok', null, null, {
    referred_id: referredUserId, tickets_reward: ticketReward,
  });
  console.info(`[REFERRAL] Activated — referrer=${pendingRef.referrer_id} referred=${referredUserId} tickets=${ticketReward}`);
  ...
  if (refUser?.tg_id) {
    await sendBotMessage(
      refUser.tg_id,
      `🎟️ <b>تم تفعيل مكافأة الإحالة!</b>\n\nأصبح أحد أصدقائك الذين دعوتهم مستخدماً نشطاً.\n\n🎁 تم إضافة <b>${ticketReward} تذكرة مسابقة</b> إلى رصيدك.\n\nاستمر بدعوة المزيد لترفع ترتيبك! 🏆`
    );
  }
}
...
async function handleLoad(userId) {
  ...
  return {
    ...
    my_tickets: await (async () => {
      try {
        const s = (await sql(`SELECT id FROM competition_seasons WHERE end_date > NOW() ORDER BY end_date ASC LIMIT 1`))[0];
        if (!s) return 0;
        const t = (await sql(`SELECT tickets FROM competition_tickets WHERE season_id=$1 AND user_id=$2`, [s.id, userId]))[0];
        return parseInt(t?.tickets) || 0;
      } catch { return 0; }
    })(),
  };
}
...
async function handleGetState(userId) {
  ...
  return {
    ...
    monetag_daily_limit:   CFG.MONETAG_DAILY_LIMIT,
  };
}
...
async function handleRewardAd(userId, sessionId, ipHash, fpHash, body, rawNonce) {
  ...
  const isPartial   = !!body?.data?.partial; // نصف جائزة إذا < 30 ثانية

  // [1] Nonce verification — مع retry تلقائي عند انتهاء الصلاحية
  let nonceResult = await consumeAdNonce(sessionId, rawNonce, userId, ipHash, fpHash);

  if (nonceResult === 'expired' || nonceResult === 'not_issued' || nonceResult === 'missing') {
    // انتهت صلاحية الـ nonce — أعد إصداره تلقائياً وأعد المحاولة
    const newSigned = await issueAdNonce(sessionId, userId, ipHash);
    if (newSigned) {
      nonceResult = await consumeAdNonce(sessionId, newSigned, userId, ipHash, fpHash);
    }
  }
  ...
  // [5] Timing check — رفض الإعلانات القصيرة جداً فقط
  if (adElapsedMs > 0 && adElapsedMs < CFG.AD_MIN_DURATION_MS) {
    logRewardBlock('timing_invalid', userId, { elapsed_ms: adElapsedMs });
    return { ok: false, error: 'ad_duration_invalid' };
  }

  // [6] Replay detection — نافذة 15 ثانية
  ...
  // [7] Atomic log — تسجيل الإعلان (tickets_per_ad بدل points)
  const ticketsToAward = isPartial
    ? Math.max(1, Math.floor(CFG.TICKETS_PER_AD / 2))
    : CFG.TICKETS_PER_AD;
  ...
  const lr = await sql(
    ...
    [userId, ticketsToAward, CFG.ADS_DAILY_LIMIT]
  );
  const watched = parseInt(lr[0].count) || 0;

  const nextCooldownMs = _randomAdCooldownMs();

  // تحديث إحصائيات الإعلانات + إضافة USDT
  const usdtReward = isPartial
    ? CFG.USDT_PER_AD / 2
    : (CFG.USDT_PER_AD || 0.001);

  await sql(
    `UPDATE users
     SET ads_watched_total=ads_watched_total+1,
         ad_last_reward=NOW(),
         ad_next_cooldown_ms=$2,
         usdt_balance=usdt_balance+$3,
         updated_at=NOW()
     WHERE id=$1`,
    [userId, nextCooldownMs, usdtReward]
  );
  ...
  let totalTickets = 0;
  try {
    const ticketRes = await grantCompTickets(userId, ticketsToAward);
    if (ticketRes.ok) totalTickets = ticketRes.tickets;
  } catch (_) {}

  await writeAudit(userId, sessionId, 'reward_ad', 'ok', ipHash, fpHash, {
    watched, tickets_earned: ticketsToAward,
  });

  const ur = await sql(`SELECT points, usdt_balance FROM users WHERE id=$1`, [userId]);
  return {
    ok:              true,
    partial:         isPartial,
    tickets_awarded: ticketsToAward,
    ...
    usdt_balance:    parseFloat(ur[0]?.usdt_balance) || 0,
...
async function handleSubmitWithdraw(userId, body, rawNonce, sessionId, fpHash, ipHash) {
  ...
  const r = await sql(`SELECT points, usdt_balance, level, first_withdraw_done FROM users WHERE id=$1`, [userId]);
  if (!r.length) return { ok: false, error: 'user_not_found' };

  const pts       = parseInt(r[0].points) || 0;
  const usdt      = parseFloat(r[0].usdt_balance) || 0;
  const level     = parseInt(r[0].level)  || 1;
  const firstDone = !!r[0].first_withdraw_done;

  // Determine requirements (USDT-based)
  if (!firstDone) {
    if (usdt < CONFIG.withdraw.first_min) {
      return {
        ok: false,
    ...
    if (usdt < CONFIG.withdraw.normal_min) {
      return { ok: false, error: 'insufficient_balance', required: CONFIG.withdraw.normal_min, have: usdt };
    }
  }

  // خصم USDT من الرصيد
  await sql(`UPDATE users SET usdt_balance=GREATEST(0, usdt_balance-$1), updated_at=NOW() WHERE id=$2`, [usdt, userId]);
  const wr = await sql(
    `INSERT INTO withdrawals(user_id, pts, ton_amount, address, method, status) VALUES($1,$2,$3,$4,'ton','pending') RETURNING id`,
    [userId, Math.round(usdt * 100000), usdt, address]
  );
  ...
  const nb = (await sql(`SELECT usdt_balance, tg_id FROM users WHERE id=$1`, [userId]))[0];
  await writeAudit(userId, sessionId, 'submit_withdraw', 'ok', ipHash, fpHash, {
    usdt, first_withdraw: !firstDone,
  });
