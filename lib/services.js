...
async function handleRewardAd(userId, sessionId, ipHash, fpHash, body, rawNonce) {
  ...
  if (watched >= CONFIG.referral.min_active_ads) {
    ...
    // تحقق من الشرط الثاني (النقاط) إذا لم تستوفَ الإعلانات بعد
    try {
      if (await isReferralActive(userId)) await activatePendingReferral(userId);
    } catch (_) {}
  }
...
async function handleClaimGift(userId, rawNonce, sessionId, fpHash, ipHash) {
  ...
  // تفعيل الإحالة المعلّقة إذا استوفى المستخدم الشرط بعد الهدية
  try { if (await isReferralActive(userId)) await activatePendingReferral(userId); } catch (_) {}
