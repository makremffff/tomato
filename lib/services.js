...
async function handleRewardAd(userId, sessionId, ipHash, fpHash, body, rawNonce) {
  ...
  if (adElapsedMs > 0) {
    if (adElapsedMs < CFG.AD_MIN_DURATION_MS) {
      // إعلان قصير جداً — قد يكون مشكلة شبكة أو جهاز بطيء
      // نرفض الطلب بدون إضافة risk عالي (مجرد log)
      await addRisk(userId, 'ad_too_fast_client', ipHash, fpHash, 3);
  ...
  // [6] Replay detection — نافذة 15 ثانية (المستخدم الطبيعي لن يعيد في 15 ثانية)
  ...
  if (recentReward.length) {
    // ضغطة مزدوجة أو retry سريع — نرفض بدون risk عالي
    await addRisk(userId, 'reward_ad_replay', ipHash, fpHash, 5);
...
async function handleClaimAdsgramTask(userId, sessionId, ipHash, fpHash, rawNonce) {
  ...
  if (elapsed < CFG.ADSGRAM_TASK_MIN_WATCH_MS) {
    ...
    await addRisk(userId, 'adsgram_task_too_fast', ipHash, fpHash, 5);
