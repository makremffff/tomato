...
async function handleStartTaddyAd(userId, sessionId, ipHash, fpHash) {
  ...
  const nonce = await issueTaddyNonce(sessionId);
  if (!nonce) return { ok: false, error: 'session_invalid' };
...
async function handleAdsgramCallback(req, res) {
  ...
  const queryStr = qStart >= 0 ? rawUrl.slice(qStart + 1) : '';
  const query   = new URLSearchParams(queryStr);

  // ── [V-01 FIX] HMAC Secret Verification ────────────────────────
  const secret = process.env.ADSGRAM_REWARD_SECRET;
  if (secret) {
    const receivedSig = req.headers['x-adsgram-signature']
                     || query.get('sig')
                     || '';
    if (!receivedSig) {
      res.status(403).end('missing_signature');
      return;
    }
    ...
    const expectedSig = createHmac('sha256', secret)
      ...
      .digest('hex');
    try {
      const a = Buffer.from(receivedSig.toLowerCase().padEnd(64,'0').slice(0,64), 'hex');
      const b = Buffer.from(expectedSig.slice(0,64), 'hex');
      if (a.length !== b.length || !timingSafeEqual(a, b)) {
        await writeAudit(0, 'adsgram_cb', 'adsgram_callback', 'invalid_signature',
          hashIp(getIp(req)), null, { received: receivedSig.slice(0, 8) });
        res.status(403).end('invalid_signature');
        return;
      }
    } catch (_) {
      res.status(403).end('invalid_signature');
