...
const {
  issueAdNonce, issueTaddyNonce, issueNonce, consumeNonce, consumeAdNonce, consumeTaddyNonce,
  writeAudit, addRisk, checkMultiAccount,
  genSignedSessionId, verifySessionId,
} = require('./security');
...
function handleGetConfig() {
  return {
    ...
    taddy_ads: {
      daily_limit:   CONFIG.taddy_ads.daily_limit,
      points_per_ad: CONFIG.taddy_ads.points_per_ad,
      cooldown_ms:   CONFIG.taddy_ads.cooldown_ms,
    },
  };
}
...
async function handleCreateSession(body, ipHash, fpHash) {
  ...
  try {
    ...
    // ✅ [SEC] Session ID موقَّع بـ HMAC مشتق من BOT_TOKEN — لا يمكن تزويره
    sessionId = genSignedSessionId();
...
async function handleLoad(userId) {
  ...
  // ── Taddy ads today count ─────────────────────────────────────
  const taddyRow = await sql(
    `SELECT count, points_earned FROM taddy_ad_logs WHERE user_id=$1 AND log_date=CURRENT_DATE`,
    [userId]
  );
  const taddyWatchedToday = parseInt(taddyRow[0]?.count) || 0;
  ...
  return {
    ...
    earned_today:      (parseInt(ad.points_earned) || 0) + (parseInt(taddyRow[0]?.points_earned) || 0),
    ...
    taddy_ads: {
      watched_today: taddyWatchedToday,
      daily_limit:   CFG.TADDY_DAILY_LIMIT,
      remaining:     Math.max(0, CFG.TADDY_DAILY_LIMIT - taddyWatchedToday),
    },
...
async function handleGetState(userId) {
  ...
  const taddyPoll = (await sql(`SELECT count, points_earned FROM taddy_ad_logs WHERE user_id=$1 AND log_date=CURRENT_DATE`, [userId]))[0] || { count: 0, points_earned: 0 };
  const taddyWatchedPoll = parseInt(taddyPoll.count) || 0;
  const taddyEarnedPoll  = parseInt(taddyPoll.points_earned) || 0;
  ...
  return {
    ...
    earned_today:      (parseInt(ad.points_earned) || 0) + taddyEarnedPoll,
    ad_cooldown_ms,
    ads_daily_limit: CFG.ADS_DAILY_LIMIT,
    taddy_watched_today: taddyWatchedPoll,
    taddy_remaining:     Math.max(0, CFG.TADDY_DAILY_LIMIT - taddyWatchedPoll),
...
async function handleStartAd(userId, sessionId, ipHash, fpHash) {
  ...
  // ✅ [SEC] سجّل وقت البداية server-side — يُستخدم في reward_ad للتحقق من التوقيت الفعلي
...
async function handleRewardAd(userId, sessionId, ipHash, fpHash, body, rawNonce) {
  ...
  // ✅ [SEC-FIX] [5a] Server-side timing — تحقق من وقت start_ad الحقيقي على السيرفر
  // يمنع البوت من إرسال elapsed_ms مزوّر دون أن ينتظر فعلياً
  const _sessionStartRow = (await sql(
    `SELECT ad_started_at FROM sessions WHERE id=$1 AND is_revoked=FALSE`,
    [sessionId]
  ))[0];
  if (!_sessionStartRow?.ad_started_at) {
    await addRisk(userId, 'reward_ad_no_server_start', ipHash, fpHash, 20);
    logRewardBlock('no_server_start_time', userId, {});
    return { ok: false, error: 'ad_not_started' };
  }
  const _serverElapsedMs = Date.now() - new Date(_sessionStartRow.ad_started_at).getTime();
  if (_serverElapsedMs < CFG.AD_MIN_DURATION_MS) {
    await addRisk(userId, 'reward_ad_server_too_fast', ipHash, fpHash, 25);
    logRewardBlock('server_timing_violation', userId, { server_ms: _serverElapsedMs });
    return { ok: false, error: 'ad_duration_invalid' };
  }
  // امسح وقت البداية فوراً — يمنع reuse في حالة race condition
  await sql(`UPDATE sessions SET ad_started_at=NULL WHERE id=$1`, [sessionId]);

  // [5b] Timing check على elapsed_ms الحقيقي من controller.show()
  //   مُلزم الآن — لا يمكن تخطيه بإرسال 0 أو null
  //   < AD_MIN_DURATION_MS  → رُفض (تلاعب)
  //   30000 – 31999 ms      → إعلان بدون تفاعل كامل → 50% جائزة
  //   ≥ 32000 ms            → تفاعل كامل → جائزة كاملة
  let isPartialReward = false;
  // ✅ [SEC-FIX] elapsed_ms إلزامي — إرسال 0 أو undefined = رفض فوري
  if (!adElapsedMs || adElapsedMs < CFG.AD_MIN_DURATION_MS) {
    await addRisk(userId, 'ad_elapsed_missing_or_fast', ipHash, fpHash, 10);
...
// نفس نمط start_ad / reward_ad لكن بجدول taddy_ad_logs
// ═══════════════════════════════════════════════════════════════════

async function handleStartTaddyAd(userId, sessionId, ipHash, fpHash) {
  const adR = await sql(
    `SELECT count FROM taddy_ad_logs WHERE user_id=$1 AND log_date=CURRENT_DATE`,
    [userId]
  );
  if ((parseInt(adR[0]?.count) || 0) >= CFG.TADDY_DAILY_LIMIT) {
    return { ok: false, error: 'daily_limit_reached' };
  }
  const coolRow = (await sql(`SELECT taddy_last_reward, taddy_next_cooldown_ms FROM users WHERE id=$1`, [userId]))[0];
  if (coolRow?.taddy_last_reward) {
    const cooldownMs = parseInt(coolRow.taddy_next_cooldown_ms) || CFG.TADDY_COOLDOWN_MS;
    const elapsed = Date.now() - new Date(coolRow.taddy_last_reward).getTime();
    if (elapsed < cooldownMs) {
      return { ok: false, error: 'cooldown_active', wait_ms: cooldownMs - elapsed };
    }
  }
  const nonce = await issueTaddyNonce(sessionId, userId, ipHash);
  if (!nonce) return { ok: false, error: 'session_invalid' };
  await writeAudit(userId, sessionId, 'start_taddy_ad', 'ok', ipHash, fpHash, {});
  return { ok: true, taddy_nonce: nonce };
}

async function handleRewardTaddyAd(userId, sessionId, ipHash, fpHash, body, rawNonce) {
  // [1] Nonce verification
  const nonceResult = await consumeTaddyNonce(sessionId, rawNonce, userId, ipHash, fpHash);
  if (nonceResult !== 'ok') {
    logRewardBlock(`nonce_${nonceResult}`, userId, {});
    return { ok: false, error: `nonce_${nonceResult}` };
  }
  ...
  const uR = await sql(`SELECT is_shadow_banned FROM users WHERE id=$1`, [userId]);
  if (uR[0]?.is_shadow_banned) {
    logShadow('reward_taddy_blocked', userId, {});
    logRewardBlock('shadow_ban', userId, {});
    return { ok: false, error: 'account_review' };
  }
  // [3] Daily limit
  const adR = await sql(`SELECT count FROM taddy_ad_logs WHERE user_id=$1 AND log_date=CURRENT_DATE`, [userId]);
  const watchedToday = parseInt(adR[0]?.count) || 0;
  if (watchedToday >= CFG.TADDY_DAILY_LIMIT) {
    logRewardBlock('taddy_daily_limit', userId, { watched: watchedToday });
    return { ok: false, error: 'daily_limit_reached' };
  }
  // [4] Cooldown check
  const coolRow = (await sql(`SELECT taddy_last_reward, taddy_next_cooldown_ms FROM users WHERE id=$1`, [userId]))[0];
  ...
  const recentReward = await sql(
    `SELECT id FROM audit_log
     WHERE user_id=$1 AND action='reward_taddy_ad' AND status='ok'
       AND created_at > NOW() - INTERVAL '10 seconds'
     LIMIT 1`,
    [userId]
  );
  if (recentReward.length) {
    await addRisk(userId, 'reward_taddy_replay', ipHash, fpHash, 15);
    return { ok: false, error: 'cooldown_active', wait_ms: CFG.TADDY_COOLDOWN_MS };
  }
  // [6] Atomic reward
  const lr = await sql(
    `INSERT INTO taddy_ad_logs(user_id, log_date, count, points_earned)
     VALUES($1, CURRENT_DATE, 1, $2)
     ON CONFLICT(user_id, log_date) DO UPDATE SET
       count         = CASE WHEN taddy_ad_logs.count < $3 THEN taddy_ad_logs.count + 1         ELSE taddy_ad_logs.count         END,
       points_earned = CASE WHEN taddy_ad_logs.count < $3 THEN taddy_ad_logs.points_earned + $2 ELSE taddy_ad_logs.points_earned END,
       updated_at    = NOW()
     RETURNING count, points_earned`,
    [userId, CFG.TADDY_POINTS_PER_AD, CFG.TADDY_DAILY_LIMIT]
  );
  const watched = parseInt(lr[0].count) || 0;
  const nextCooldownMs = CFG.TADDY_COOLDOWN_MS;
  await sql(
    `UPDATE users SET points=points+$1, xp=xp+$1,
        taddy_last_reward=NOW(), taddy_next_cooldown_ms=$3, updated_at=NOW()
     WHERE id=$2`,
    [CFG.TADDY_POINTS_PER_AD, userId, nextCooldownMs]
  );
  await syncLevel(userId);

  // ── Daily missions — combined adsgram + taddy count ──────────────
  const adsgramRow = await sql(
    `SELECT COALESCE(count, 0) AS count FROM ad_logs WHERE user_id=$1 AND log_date=CURRENT_DATE`,
    [userId]
  );
  const combinedWatched = (parseInt(adsgramRow[0]?.count) || 0) + watched;
  if (combinedWatched >= 10) await upsertTask(userId, 'ads_10');
  if (combinedWatched >= 25) await upsertTask(userId, 'ads_25');

  try { await giveReferralAdCommission(userId, CFG.TADDY_POINTS_PER_AD); } catch (_) {}
  await writeAudit(userId, sessionId, 'reward_taddy_ad', 'ok', ipHash, fpHash, {
    watched, points_earned: CFG.TADDY_POINTS_PER_AD, combinedWatched,
  });
  const ur = await sql(`SELECT points FROM users WHERE id=$1`, [userId]);
  return {
    ok:              true,
    points_awarded:  CFG.TADDY_POINTS_PER_AD,
    remaining:       Math.max(0, CFG.TADDY_DAILY_LIMIT - watched),
    watchedToday:    watched,
    combinedWatched,
    earnedToday:     parseInt(lr[0].points_earned) || 0,
    points:          parseInt(ur[0]?.points) || 0,
    all_done:        watched >= CFG.TADDY_DAILY_LIMIT,
    cooldown_ms:     nextCooldownMs,
  };
}
...
async function handleAdsgramCallback(req, res) {
  ...
  // ✅ [SEC-FIX] السر مشتق من BOT_TOKEN بدل hardcoded — لا يمكن spoof حتى لو سرّب الكود
  const _adsgramSeed = (process.env.BOT_TOKEN || '') + '|adsgram-cb-v1';
  const ADSGRAM_SECRET = process.env.ADSGRAM_CALLBACK_SECRET
    || require('crypto').createHash('sha256').update(_adsgramSeed).digest('hex');
  if (ADSGRAM_SECRET) {
  ...
  try {
    ...
    // ✅ [SEC-FIX] INSERT atomic مع حماية daily limit — منع تجاوز الحد حتى في race condition
    await sql(
      `INSERT INTO ad_logs(user_id, log_date, count, points_earned)
       VALUES($1, CURRENT_DATE, 1, $2)
       ON CONFLICT(user_id, log_date) DO UPDATE SET
