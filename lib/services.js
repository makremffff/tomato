...
function _randomAdCooldownMs() {
  const options = [25000, 25000, 27000, 28000, 30000, 30000];
...
// حد يومي: 250 | جائزة: 20 نقطة | cooldown: 20 ثانية
// ══════════════════════════════════════════════════════════════════
async function handleMonetagReward(userId, sessionId, ipHash, fpHash, body) {
  ...
  const watchedToday = parseInt(adR[0]?.count) || 0;
  if (watchedToday >= CFG.MONETAG_DAILY_LIMIT) {
    return { ok: false, error: 'daily_limit_reached', watched_today: watchedToday };
  }
  ...
  // [4] Replay detection (20 seconds window)
  const recentReward = await sql(
    `SELECT id FROM audit_log
     WHERE user_id=$1 AND action='monetag_reward' AND status='ok'
       AND created_at > NOW() - INTERVAL '18 seconds'
     LIMIT 1`,
    [userId]
  );
  if (recentReward.length) {
  ...
  // [5] Atomic insert/update monetag_logs
  const lr = await sql(
  ...
  // [6] Update user points and monetag_last_reward
  await sql(
    `UPDATE users
     SET points=points+$1, xp=xp+$1,
  ...
  // [7] Referral commission (5% like regular ads)
  ...
  await writeAudit(userId, sessionId, 'monetag_reward', 'ok', ipHash, fpHash, {
    watched, points_earned: CFG.MONETAG_REWARD,
  });
  ...
  return {
    ...
    remaining:      Math.max(0, CFG.MONETAG_DAILY_LIMIT - watched),
    cooldown_ms:    CFG.MONETAG_COOLDOWN_MS,
    points:         parseInt(ur[0]?.points) || 0,
    all_done:       watched >= CFG.MONETAG_DAILY_LIMIT,
  };
}
