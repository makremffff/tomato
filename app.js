...
══════════════════════════════════════════════════════ */
  ...
  /* ===== "تصفح واربح" — صفحة كاملة مستقلة، دفعات كل 5 ثواني (التحقق الفعلي من الوقت من السيرفر) =====
     🛡️ سكربت Adcash ما يتحمّل نهائياً بأي مكان تاني بالتطبيق — يتحمّل فقط هون، ولحظة موافقة
     المستخدم فعلياً، ويتشال من الصفحة تماماً أول ما يخرج المستخدم (زر الخروج أو انتهاء الجلسة) */
  let surfState = null; // { nonce, tick, totalTicks, tickSeconds, timer, earned }
  let surfPreviousPageId = 'rewards';
  let adcashScriptEl = null;
  const SURF_CONSENT_KEY = 'surfConsentAcknowledged';
  const ADCASH_ZONE_ID = 'la4zewigtl';

  function loadAdcashScript(){
    if (adcashScriptEl || window.aclib) return; // محمّل أصلاً
    ...
    let attempts = 0;
    const maxAttempts = 20; // ~5 ثواني
    function tryInit(){
      // لو المستخدم خرج من الصفحة قبل ما يخلص التحميل، منوقف — ما منشغّل إعلان بصفحة تانية
      if (!surfState && !document.getElementById('page-surf').classList.contains('active')) return;
      if (window.aclib && typeof window.aclib.runAutoTag === 'function'){
        try { window.aclib.runAutoTag({ zoneId: ADCASH_ZONE_ID }); }
        catch(e){ console.warn('[adcash] runAutoTag error', e); }
        return;
      }
      attempts++;
      if (attempts >= maxAttempts){
        console.warn('[adcash] aclib لم يتحمّل — تم تخطي AutoTag');
        return;
      }
  ...
  function unloadAdcashScript(){
    if (adcashScriptEl && adcashScriptEl.parentNode){
      adcashScriptEl.parentNode.removeChild(adcashScriptEl);
    }
    adcashScriptEl = null;
    try { delete window.aclib; } catch(e){ window.aclib = undefined; }
  }
  ...
  function openSurfPage(){
    ...
    document.getElementById('surfAdPlaceholderText').textContent = t('surf.adPlaceholder');
  ...
  async function beginSurfSession(){
    ...
    document.getElementById('surfAdPlaceholderText').textContent = t('surf.loadingAds');
    try{
      ...
      const totalSeconds = surfState.totalTicks * surfState.tickSeconds;
      document.getElementById('surfCountdownDisplay').textContent = totalSeconds;
      loadAdcashScript(); // 🛡️ التحميل يبلش هون بالضبط — بعد الموافقة وبدء الجلسة، وبهاي الصفحة فقط
      document.getElementById('surfAdPlaceholderText').textContent = t('surf.adPlaceholder');
  ...
  async function surfClaimNextTick(){
    ...
    try{
      ...
      const remaining = (surfState.totalTicks - surfState.tick) * surfState.tickSeconds;
      document.getElementById('surfCountdownDisplay').textContent = remaining;
